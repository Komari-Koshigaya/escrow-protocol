...
public class BitcoinBase58 {
    ...
    public static String encodeChecked(int version, byte[] payload) {
        ...
        byte[] dataForHash = new byte[1 + payload.length];
        addressBytes[0] = (byte) version;
        System.arraycopy(payload, 0, addressBytes, 1, payload.length);
        System.arraycopy(addressBytes, 0, dataForHash, 0, dataForHash.length);
        byte[] checksum = Hash.sha256(Hash.sha256(dataForHash));
        System.arraycopy(checksum, 0, addressBytes, payload.length + 1, 4);
        return BitcoinBase58.encode(addressBytes);
    }
    ...
    public static byte[] decode(String input) throws ParseException {
        ...
        for (int i = 0; i < input.length(); ++i) {
            ...
            if (digit < 0) {
                throw new ParseException(String.valueOf(c), i);
            }
